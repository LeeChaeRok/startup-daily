...
```bash
mkdir -p $(dirname .omc/state/team/startup-daily/workers/worker-3/.ready) && touch .omc/state/team/startup-daily/workers/worker-3/.ready
```
...
- **Worker**: worker-3
- **Agent Type**: claude
- **Environment**: OMC_TEAM_WORKER=startup-daily/worker-3
...
- **Inbox**: Read .omc/state/team/startup-daily/workers/worker-3/inbox.md for new instructions
- **Heartbeat**: Update .omc/state/team/startup-daily/workers/worker-3/heartbeat.json every few minutes:
  ```json
...
- Path: .omc/state/team/startup-daily/workers/worker-3/done.json
...
2. Write an ACK file: .omc/state/team/startup-daily/workers/worker-3/shutdown-ack.json
3. Exit
