...
```bash
mkdir -p $(dirname .omc/state/team/startup-daily/workers/worker-1/.ready) && touch .omc/state/team/startup-daily/workers/worker-1/.ready
```
...
- **Worker**: worker-1
- **Agent Type**: claude
- **Environment**: OMC_TEAM_WORKER=startup-daily/worker-1
...
- **Inbox**: Read .omc/state/team/startup-daily/workers/worker-1/inbox.md for new instructions
- **Heartbeat**: Update .omc/state/team/startup-daily/workers/worker-1/heartbeat.json every few minutes:
  ```json
...
- Path: .omc/state/team/startup-daily/workers/worker-1/done.json
...
2. Write an ACK file: .omc/state/team/startup-daily/workers/worker-1/shutdown-ack.json
3. Exit
