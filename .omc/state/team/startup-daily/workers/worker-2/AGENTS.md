...
```bash
mkdir -p $(dirname .omc/state/team/startup-daily/workers/worker-2/.ready) && touch .omc/state/team/startup-daily/workers/worker-2/.ready
```
...
- **Worker**: worker-2
- **Agent Type**: claude
- **Environment**: OMC_TEAM_WORKER=startup-daily/worker-2
...
- **Inbox**: Read .omc/state/team/startup-daily/workers/worker-2/inbox.md for new instructions
- **Heartbeat**: Update .omc/state/team/startup-daily/workers/worker-2/heartbeat.json every few minutes:
  ```json
...
- Path: .omc/state/team/startup-daily/workers/worker-2/done.json
...
2. Write an ACK file: .omc/state/team/startup-daily/workers/worker-2/shutdown-ack.json
3. Exit
